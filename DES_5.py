...
# 计算器模式（CTR）
def CTR(origbit, keys, iv, mode):  # mode=True表示加密，mode=False表示解密
    counter = int.from_bytes(iv.encode(), 'big')  # 将 IV 转换为整数
    newbit = ''
    
    for i in range(0, len(origbit), 64):
        block = origbit[i:i + 64]
        counter_block = format(counter, '064b')  # 将计数器格式化为64位二进制字符串
        keystream = DES(counter_block, keys, True)  # 加密计数器
        newbit += xor(block, keystream)  # 异或得到结果
        counter += 1  # 增加计数器
        
    return newbit

# 输出反馈模式（OFB）
def OFB(origbit, keys, iv, mode):  # mode=True表示加密，mode=False表示解密
    output = iv
    newbit = ''
    
    for i in range(0, len(origbit), 64):
        output = DES(output, keys, True)  # 加密输出块
        block = origbit[i:i + 64]
        newbit += xor(block, output)  # 异或得到结果
        
    return newbit

# 计算反馈模式（CFB）
def CFB(origbit, keys, iv, mode):  # mode=True表示加密，mode=False表示解密
    output = iv
    newbit = ''
    
    for i in range(0, len(origbit), 64):
        output = DES(output, keys, True)  # 加密当前输出块
        block = origbit[i:i + 64]
        newbit += xor(block, output)  # 异或得到结果
        # 通过当前密文更新输出块
        output = xor(block, output)  # 将当前明文块与输出块异或更新输出块
...
if __name__ == '__main__':
    ...
    with open(file_path, 'r', encoding='utf-8') as fp:
        ...
        mode = int(input('你需要加密还是解密？（1加密，0解密,现在只能加密）'))
        mode = bool(mode)
        
        mthd = int(input('你要使用哪种模式？（1表示ECB，2表示CBC，3表示CTR，4表示OFB，5表示CFB）'))
        ...
        # 处理CTR模式
            ...
            iv = str2bin(ivstr)[:64]
            origbin = origtxt if mode == 0 else str2bin(origtxt)
            newbinCTR = CTR(origbin, keys, iv, mode)
            newbinCTR = bin2str(newbinCTR) if mode == 0 else newbinCTR
            with open('newCTR.txt', 'w', encoding='utf-8') as outfile:
                outfile.write(newbinCTR)
        ...
        # 处理OFB模式
            ...
            iv = str2bin(ivstr)[:64]
            origbin = origtxt if mode == 0 else str2bin(origtxt)
            newbinOFB = OFB(origbin, keys, iv, mode)
            newbinOFB = bin2str(newbinOFB) if mode == 0 else newbinOFB
            with open('newOFB.txt', 'w', encoding='utf-8') as outfile:
                outfile.write(newbinOFB)
        ...
        # 处理CFB模式
            ...
            iv = str2bin(ivstr)[:64]
            origbin = origtxt if mode == 0 else str2bin(origtxt)
            newbinCFB = CFB(origbin, keys, iv, mode)
            newbinCFB = bin2str(newbinCFB) if mode == 0 else newbinCFB
            with open('newCFB.txt', 'w', encoding='utf-8') as outfile:
                outfile.write(newbinCFB)
